...
namespace DataTables.AspNetCore.Mvc
{
    ...
    public class GridBuilder<T> : IHtmlContent where T : class
    {
        ...
        public void WriteTo(TextWriter writer, HtmlEncoder encoder)
        {
            ...
            sb.AppendLine("    if ($(\"#" + this.Grid.Name + "\").length == 0) {");
            if (!string.IsNullOrWhiteSpace(this.Grid.ClassName))
            {
                sb.AppendLine("        document.write('<table id=\"" + this.Grid.Name + "\" class=\"display "+ this.Grid.ClassName + "\" cellspacing=\"0\" width=\"100 % \"></table>')");
            }
            else
            {
                sb.AppendLine("        document.write('<table id=\"" + this.Grid.Name + "\" class=\"display\" cellspacing=\"0\" width=\"100 % \"></table>')");
            }
            sb.AppendLine("      }");
